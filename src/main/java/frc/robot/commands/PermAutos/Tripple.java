...
public class Tripple extends InstantCommand {
  ...
  public Tripple(
    ...
    blueCommandSequence =
    Commands.sequence(
    new DropHighPosition(pDislocator, pElbow, pShoulder, pTravelator).withTimeout(1.5), // with droplow the timeout is 3
    new GrabOpen(wrist).withTimeout(.3),
    Commands.parallel(
    ...
    new GrabClose(wrist).withTimeout(.3),
    Commands.parallel(
      new DriveAutoGyro(drivetrain, .7, 0).withTimeout(1.2).andThen(
      new GrabOpen(pWrist).withTimeout(0).andThen(
      new DriveAutoGyro(drivetrain, -.7, 0).withTimeout(1))),
      new TuckPosition(pDislocator, pElbow, pShoulder, pTravelator)),
    Commands.parallel(
      new DriveAutoGyro(drivetrain, 0, -135).withTimeout(0.6).andThen(
      new DriveAutoGyro(drivetrain, .6, -135).withTimeout(0.8)),
      new TuckPosition(pDislocator, pElbow, pShoulder, pTravelator).withTimeout(.4).andThen(
      new GrabPosition(pDislocator, pElbow, pShoulder, pTravelator).withTimeout(1.5).andThen(
      new GrabClose(pWrist).withTimeout(.3)))),
    Commands.parallel(
      new TuckPosition(pDislocator, pElbow, pShoulder, pTravelator).withTimeout(2),
      new DriveAutoGyro(drivetrain, -.6, -135).withTimeout(0.8))).andThen(
      new DriveAutoGyro(drivetrain, 0, 0).withTimeout(0.4).andThen(
      new DriveAutoGyro(drivetrain, .6, 0).withTimeout(1.5)),
    Commands.parallel(
      new DropLowPosition(pDislocator, pElbow, pShoulder, pTravelator).withTimeout(3).andThen(
      new GrabOpen(pWrist))));
